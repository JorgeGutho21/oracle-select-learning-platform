...
- **Diagnósticos pedagógicos.** Tienen códigos propios y nunca imitan códigos ORA. Se clasifican en sintaxis, columna, tabla, alias, fuera de alcance, seguridad, límite y operación, e indican línea y columna. Aplican los límites de LAB_SPEC: 4000 caracteres, 500 tokens, 12 elementos y 8 niveles de paréntesis. `SELECT nombre salario` es válido y se señala con el aviso de posible coma ausente (LAB10).

...
- **Secretos:**
  - `.gitignore` y `.vercelignore` excluyen `.secrets/`, `PASSWORD.txt`, carteras, certificados y claves.
  - Ningún valor secreto aparece en el historial de la rama ni en lo que sirve producción.
  - Las variables se cargaron en Vercel por la entrada estándar, como _sensitive_.
- **Supabase real:**
  - Migración aplicada a través del pooler IPv4, porque la conexión directa es solo IPv6.
  - RLS verificado con la clave publishable: `42501` en las cinco tablas y en las funciones.
  - El código acepta las claves modernas `sb_publishable_…` y `sb_secret_…`, y nunca entrega una clave secreta al navegador.
- **Oracle Cloud:**
  - Autonomous Database 19c Always Free en sa-bogota-1, con mTLS.
  - La cartera llega a `oracledb` como `walletContent` desde variables de servidor.
  - `scripts/oracle-cloud.mjs` creó `SQL_LAB_OWNER.EMPLEADOS` (dataset `empleados-select-v1`) y `SQL_LAB_READER`, con solo `CREATE SESSION` y `READ`.
  - ADMIN se rechaza como usuario de la aplicación.
- **Subtítulos del video resumen:**
  - Transcripción local con Whisper small, revisada a mano: términos SQL, errores de reconocimiento y una frase duplicada.
  - Pista WebVTT de 110 subtítulos y transcripción en UTF-8.
  - La revisión se hizo contra las diapositivas y la sincronía con los fotogramas; no se escuchó el audio.
- **Vercel:**
  - Variables configuradas y vista previa probada 22/22.
  - Los orígenes de la barra de Vercel se admiten en la CSP solo en vistas previas.
  - Producción en `sql-select-lab.vercel.app`, probada 24/24 ([DEPLOYMENT.md](DEPLOYMENT.md)).

### Hallazgos de la Fase 11

| ID     | Severidad              | Hallazgo                                                                                                                                                                                                             | Estado                                                                                                                                                       |
| ------ | ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| F11-01 | MAJOR (seguridad)      | `PASSWORD.txt` en la raíz, con claves de Supabase, no estaba ignorado por Git. `.secrets/`, donde está la cartera de Oracle, no estaba en `.vercelignore`: un `vercel deploy` la habría subido.                      | **Corregido** antes de cualquier integración (commit `38ddc02`). Ninguno de los dos llegó a Git ni a Vercel.                                                 |
| F11-02 | MAJOR (integración)    | El código solo leía `SUPABASE_SERVICE_ROLE_KEY` y `NEXT_PUBLIC_SUPABASE_ANON_KEY`, pero el proyecto real usa las claves nuevas. `.env.local` tenía además líneas sin nombre que el lector de `.env` malinterpretaba. | **Corregido:** se leen primero los nombres modernos, con protección contra claves privilegiadas, y `.env.local` quedó normalizado sin perder valores.        |
| F11-03 | MAJOR (integración)    | El adaptador Oracle no admitía la cartera mTLS que exige Autonomous Database con acceso desde cualquier lugar.                                                                                                       | **Corregido:** `ORACLE_WALLET_PEM_BASE64` y `ORACLE_WALLET_PASSWORD` se pasan como `walletContent`. Pruebas unitarias y 21/21 pruebas reales contra la nube. |
| F11-04 | MINOR (vistas previas) | La barra de Vercel, que se inyecta en las vistas previas, chocaba con la CSP y dejaba un error en la consola.                                                                                                        | **Corregido** solo para compilaciones de vista previa; la CSP de producción no cambia.                                                                       |
| F11-05 | MINOR (accesibilidad)  | Safari no activa la pista `default` si la preferencia de subtítulos del sistema es «Automático»; la ofrece en el menú del reproductor.                                                                               | Comportamiento del navegador. En Chromium y Edge la pista aparece activada; en WebKit la E2E la activa como el menú y comprueba los subtítulos.              |
| F11-06 | MINOR (pruebas)        | ESLint revisaba los guiones locales de QA de `output/`, ignorado por Git.                                                                                                                                            | **Corregido:** `output/**` está excluido.                                                                                                                    |
| F10-01 | MAJOR (WCAG 1.2.2)     | V02 no tenía subtítulos.                                                                                                                                                                                             | **Cerrado** con la pista revisada.                                                                                                                           |

Estado de los MINOR de la Fase 9:

- M-03 (sin límite de peticiones por IP en el laboratorio) se mantiene a propósito: en un aula, todos los estudiantes comparten una IP pública y un límite por IP los bloquearía durante la exposición.
- M-05 (canal Broadcast público) sigue documentado; solo transporta la revisión.
- Los demás siguen como en la Fase 10.

### Verificaciones de la Fase 11
...
| `npm run test:unit`                  | 26 archivos, 426 pruebas correctas; 21 contra Oracle real.                                                                                                                                                                                    |
| Oracle Cloud (integración)           | 21/21 con `scripts/with-oracle-cloud.mjs`: salud, dataset, privilegios, LAB11 y M10.                                                                                                                                                          |
| `npm run build`                      | Correcto en local, desde cero, y en Vercel.                                                                                                                                                                                                   |
| `npm run test:e2e`, pasada completa  | **482/489 en 1,1 h** con un solo worker y la CPU al 100 % (más de la mitad, procesos ajenos: WMI, antivirus y varios `git`). Los 7 fallos fueron plazos agotados en WebKit. Repetidos aislados: 40/42, y los dos restantes también por plazo. |
| M06 en WebKit                        | Pasos reproducidos a mano 4/4: el orden de las piezas y los encabezados son correctos. El fallo era el plazo de 30 s: cada clic tardaba 0,6–2,5 s con la CPU saturada.                                                                        |
| Sala sobre el Supabase real          | Suite de la sala 15/15 en los tres navegadores. Realtime conectado: unos 2 s por cambio en local y unos 3 s desde Vercel.                                                                                                                     |
| Laboratorio y M10 sobre Oracle Cloud | E2E de laboratorio y M08–M10 40/42. Los dos fallos de Edge pasaron la lógica en 4,5 y 6,4 s y agotaron el plazo al cerrar el navegador.                                                                                                       |
| QA visual sobre producción           | 0 hallazgos: 13 tamaños × 12 rutas, 12 tamaños × 16 escenas y axe WCAG 2.2 AA a 1440 y 390 px.                                                                                                                                                |
| Estados interactivos y escenas       | 0 hallazgos: paleta, M01–M10, CodeMirror, laboratorio, consola del profesor con QR y ranking, móvil; axe de escenas a 390, 180 y 720 px.                                                                                                      |
| Vista previa de Vercel               | Prueba de humo 22/22, incluida la sala completa con Supabase real.                                                                                                                                                                            |
| Producción                           | Prueba de humo 24/24.                                                                                                                                                                                                                         |
| Secretos en producción               | Ningún valor secreto en los 31 archivos servidos; sin mapas de código; rutas sensibles responden 404.                                                                                                                                         |

### Tabla final

| Elemento              | Resultado            | Evidencia o limitación                                                                                                                                   |
| --------------------- | -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Home                  | PASS                 | Producción 24/24 y QA visual 0.                                                                                                                          |
| Learn                 | PASS                 | Lecciones, progreso y videos al inicio y al final.                                                                                                       |
| Presentation          | PASS                 | 16 escenas, teclado y lienzo 16:9 exacto.                                                                                                                |
| Search                | PASS                 | Ctrl+K en producción.                                                                                                                                    |
| Modules               | PASS                 | QA visual y E2E.                                                                                                                                         |
| Resources             | PASS                 | QA visual y E2E.                                                                                                                                         |
| Intro video           | PASS                 | Se reproduce en producción (720×1280), con subtítulos incrustados.                                                                                       |
| Summary video         | PASS                 | Se reproduce en producción y en la escena 14.                                                                                                            |
| Captions              | PASS                 | 110 subtítulos activos en producción, sincronizados (50 s y 225 s comprobados). Revisados contra las diapositivas; conviene una escucha final del autor. |
| Lab                   | PASS                 | Producción: «Oracle Database 19 (19.33.0.1.0)», con `ORA-01476` real.                                                                                    |
| Oracle real           | PASS                 | Oracle Cloud con mTLS, cuenta lectora mínima y 21/21 pruebas reales.                                                                                     |
| M01                   | PASS                 | E2E y producción.                                                                                                                                        |
| M02                   | PASS                 | E2E en los tres navegadores.                                                                                                                             |
| M03                   | PASS                 | E2E; un plazo agotado en WebKit que no se repitió al aislarlo.                                                                                           |
| M04                   | PASS                 | E2E en los tres navegadores.                                                                                                                             |
| M05                   | PASS                 | E2E en los tres navegadores.                                                                                                                             |
| M06                   | PASS                 | E2E en Chromium y Edge; en WebKit, plazos por saturación, con los pasos reproducidos 4/4.                                                                |
| M07                   | PASS                 | E2E en los tres navegadores.                                                                                                                             |
| M08                   | PASS                 | E2E en los tres navegadores.                                                                                                                             |
| M09                   | PASS                 | E2E; los fallos de Edge fueron al cerrar el navegador, con la lógica ya aprobada.                                                                        |
| M10                   | PASS                 | Calificada en Oracle Cloud en local, en la vista previa y en producción.                                                                                 |
| Presenter             | PASS                 | Vista previa con Supabase real; en producción, la clave configurada rechaza una incorrecta.                                                              |
| Create room           | PASS WITH LIMITATION | Probada en la vista previa contra el mismo Supabase. En producción falta crear una sala con la clave real, que no se escribe en pruebas automáticas.     |
| QR                    | PASS                 | En producción apunta a `https://sql-select-lab.vercel.app/…`; en la vista previa se abrió desde dos móviles.                                             |
| Join                  | PASS                 | Alias, recarga, llegada tardía y códigos no válidos.                                                                                                     |
| Supabase              | PASS                 | Proyecto real: migración, RLS y datos escritos.                                                                                                          |
| Realtime              | PASS                 | «Tiempo real conectado» y unos 2–3 s por cambio.                                                                                                         |
| Ranking               | PASS                 | Se actualiza en vivo con Supabase real.                                                                                                                  |
| Results               | PASS                 | Por rol (profesor y estudiante) con Supabase real.                                                                                                       |
| Mobile                | PASS                 | 430, 390 y 360 px, y zoom 200 %.                                                                                                                         |
| Tablet                | PASS                 | 768×1024 y 1024×768.                                                                                                                                     |
| Desktop               | PASS                 | 1920 a 1280 px.                                                                                                                                          |
| Projector             | PASS                 | 16:9 exacto de 1920×1080 a 1280×720.                                                                                                                     |
| Accessibility         | PASS                 | Axe WCAG 2.2 AA limpio, foco del Challenge y subtítulos.                                                                                                 |
| Security              | PASS                 | Cabeceras, CSP estricta en producción, RLS, secretos fuera del cliente y de Git.                                                                         |
| Build                 | PASS                 | En local y en Vercel.                                                                                                                                    |
| Unit tests            | PASS                 | 426/426.                                                                                                                                                 |
| E2E                   | PASS WITH LIMITATION | 482/489 en una pasada. Los 7 fallos fueron plazos de WebKit con la CPU saturada por procesos ajenos; ningún fallo lógico se reproduce.                   |
| Vercel Preview        | PASS                 | `sql-select-k0pfbzctk-jorge-gutierrez1.vercel.app`.                                                                                                      |
| Preview smoke test    | PASS                 | 22/22.                                                                                                                                                   |
| Production deployment | PASS                 | `dpl_61KPNiQvtuk6NAKFag4tonXAG2Fd` en <https://sql-select-lab.vercel.app>.                                                                               |
| Production smoke test | PASS WITH LIMITATION | 24/24; la sala en producción queda para una prueba del profesor con su clave.                                                                            |
...
## Prerrequisitos de despliegue (Fase 9, cumplidos en la Fase 11)

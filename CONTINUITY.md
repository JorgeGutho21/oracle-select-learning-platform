...
| Pruebas                    | En verde: lint, typecheck, formato, 250 unitarias, build y 144/144 E2E en Chromium, Edge y WebKit. No acredita Oracle, salas ni producto completo.                                                                                     |
...
Fases 1 a 4 cerradas (estabilización, núcleo del Challenge, Challenge interactivo, motor SQL y laboratorio). La rama de trabajo autorizada es `claude-finish-20260923`; no trabajar en `main` ni hacer push a `main`. Siguiente paso: esperar la instrucción del usuario. Pendientes: R1 (instancia Oracle y adaptador node-oracledb del puerto `OracleQueryExecutor`), que desbloquea P08 y la calificación final de M10; lecciones y Exposición; resultados locales en `/results`; decidir sobre la bonificación por tiempo (contradice GAME_SPEC y vale 0). Vigilar el fallo aislado de una Server Function en `next dev` descrito en `docs/CHALLENGE_STATUS.md`. Oracle no está simulado.

...
| R2                         | Cimientos y showcase hechos. Fase 5 en curso (trabajo de Codex auditado): Home, Estudio L00–L08, Exposición, buscador Ctrl/Cmd+K y Recursos implementados; la Exposición aún no sigue E01–E14 y faltan E2E de estas pantallas.                                                                                                                  |
...
| Pruebas                    | En verde: lint, typecheck, formato, 272 unitarias y build. E2E: 143/144 en Chromium, Edge y WebKit; el fallo es una espera de acción de servidor en Edge bajo `next dev` que pasa en aislado (ver `docs/CODEX_HANDOFF_AUDIT.md`). Sin E2E para Home, Estudio, Exposición, Buscador ni Recursos. No acredita Oracle, salas ni producto completo. |
...
Fases 1 a 4 cerradas. Fase 5 (Home, Estudio, Exposición, buscador y Recursos) en curso: Codex la dejó a medias; su estado exacto está en el checkpoint `8932edb` y la auditoría en `docs/CODEX_HANDOFF_AUDIT.md`. La rama de trabajo autorizada es `claude-finish-20260923`; no trabajar en `main` ni hacer push a `main`. Siguiente paso: esperar la instrucción del usuario para continuar la Fase 5 según el apartado «Qué falta» de la auditoría, empezando por reestructurar la Exposición a E01–E14. Otros pendientes: R1 (Oracle y adaptador node-oracledb), resultados locales en `/results`, permiso de uso del logotipo y decidir sobre la bonificación por tiempo (contradice GAME_SPEC y vale 0). Oracle no está simulado.

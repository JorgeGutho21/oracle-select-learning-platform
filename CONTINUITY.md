...
| R2                         | Cimientos, showcase, Fase 5 (PHASE_5 = COMPLETE) y Fase 5.1 (PHASE_5_1 = COMPLETE): Home, Modo Estudio L00–L08, Modo Exposición de dieciséis escenas 16:9, buscador Ctrl/Cmd+K, navegación de una fila, pie con identidad y reflujo con zoom 200 % en todas las rutas, incluidos laboratorio y Challenge. |
| R3–R8                      | R3 parcial: laboratorio `/lab` con análisis educativo (motor SQL único) y puerto Oracle sin conectar. R4 avanzado: M01–M09 jugables; M10 con editor y corrección estructural, a la espera de Oracle. R5–R8 pendientes; sin despliegue.                                                                    |
| Pruebas                    | En verde: lint, typecheck, formato, 302 unitarias, build y 306/306 E2E en Chromium, Edge y WebKit contra el build de producción (puerto 3200), incluido el reflujo con zoom 200 %. No acredita Oracle, salas ni producto completo.                                                                        |
...
Fases 1 a 5.1 cerradas (la 5 continúa y completa el trabajo de Codex auditado en `docs/CODEX_HANDOFF_AUDIT.md`). La rama de trabajo autorizada es `claude-finish-20260923`; no trabajar en `main` ni hacer push a `main`. Siguientes pasos (Fase 6): R1 (Oracle y adaptador node-oracledb del puerto `OracleQueryExecutor`), fase Live/Deployment con URL pública configurable para el QR, Supabase y salas (R6), resultados locales en `/results`, catálogo `/modules`, vídeos V01/V02, validación del uso público del emblema y la decisión sobre la bonificación por tiempo (contradice GAME_SPEC y vale 0). Oracle no está simulado.

Pendientes externos concretos: instancia Oracle, servicio y conectividad; configuración de Supabase; vídeos definitivos; permiso de uso del logotipo; tiempo final asignado a la exposición. Documentar qué bloquea cada pendiente y continuar el trabajo independiente.

...
| R2                         | Cimientos, showcase y Fase 5 (PHASE_5 = COMPLETE): Home, Modo Estudio L00–L08, Modo Exposición de dieciséis escenas 16:9, buscador Ctrl/Cmd+K, navegación de una fila, pie con identidad y revisión responsive y de accesibilidad.     |
...
| Pruebas                    | En verde: lint, typecheck, formato, 302 unitarias, build y 270/270 E2E en Chromium, Edge y WebKit contra el build de producción (puerto 3200). No acredita Oracle, salas ni producto completo.                                         |
...
Fases 1 a 5 cerradas (la 5 continúa y completa el trabajo de Codex auditado en `docs/CODEX_HANDOFF_AUDIT.md`). La rama de trabajo autorizada es `claude-finish-20260923`; no trabajar en `main` ni hacer push a `main`. Siguientes pasos: R1 (Oracle y adaptador node-oracledb del puerto `OracleQueryExecutor`), resultados locales en `/results`, catálogo `/modules`, vídeos V01/V02, reflujo del laboratorio a 180 px, permiso de uso del logotipo y la decisión sobre la bonificación por tiempo (contradice GAME_SPEC y vale 0). Salas, tiempo autoritativo y ranking dependen de Supabase (R6). Oracle no está simulado.
